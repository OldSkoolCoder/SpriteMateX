...
export default class Settings {
  ...
  update_colors() {
    for (let i = 0; i < this.config.colors.length; i++) {
      ...
      dom.val("#colval-" + i, ("#" + this.config.colors[i].substring(1,2) + this.config.colors[i].substring(3,4) + this.config.colors[i].substring(5,6)));
  ...
  update_custom_colors(color) {
    ...
    colvalue = "#" + ("000" + colvalue.replace(/#/g, "")).slice(-3);
    colvalue = "#" + colvalue.substring(1,2) + colvalue.substring(1,2) + colvalue.substring(2,3) + colvalue.substring(2,3) + colvalue.substring(3,4) + colvalue.substring(3,4)
